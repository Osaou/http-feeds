...
class HttpFeedRegistryImplTest {
  ...
  void defineFeed_should_accept_valid_feed() {
    ...
    assertEquals("/feed/test/", definition.path());
    assertEquals("test", definition.feed());
    assertEquals("test_table", definition.table());
    assertEquals(serviceMock, definition.feedItemService());
  }
  ...
  void getDefinedFeed_should_return_correct_feed() {
    ...
    assertEquals(expected, read.get());
  }
