...
public class HttpFeedController {
  ...
  public List<CloudEvent> getFeedItems(@PathVariable String feedName, @RequestParam(name = "lastEventId", required = false) String lastEventId, @RequestParam(name = "timeout", required = false) Long timeoutMillis) {
    ...
    final List<CloudEvent> cloudEvents = items.stream().map(CloudEventMapper::toCloudEvent).collect(Collectors.toList());
